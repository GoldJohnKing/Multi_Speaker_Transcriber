...
def test_diarizer_detects_overlap(mock_pipeline):
    """Should detect overlap between SPEAKER_00 and SPEAKER_01."""
    with patch("transcribe.models.diarizer.Diarizer._load_pipeline", return_value=mock_pipeline):
        diarizer = Diarizer(device="cpu")
        audio = _make_audio()
        result = diarizer.process(audio)

    # SPEAKER_00: 0.0-1.5 overlaps with SPEAKER_01: 1.0-2.5 → overlap 1.0–1.5
    assert len(result.overlap_regions) > 0
    overlap_start, overlap_end = result.overlap_regions[0]
    assert overlap_start == pytest.approx(1.0, abs=0.01)
    assert overlap_end == pytest.approx(1.5, abs=0.01)
...
def test_diarizer_no_overlap_when_single_speaker():
    """No overlap when only one speaker is present."""
    mock = MagicMock()
    tracks = [
        (MockTurn(0.0, 1.5), None, "SPEAKER_00"),
